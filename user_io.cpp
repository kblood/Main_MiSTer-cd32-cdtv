...
char *user_io_get_core_path(const char *suffix, int recheck)
{
	...
	if (!suffix) suffix = (!strcasecmp(name, "minimig") || !strcasecmp(name, "minimigcd")) ? "Amiga" : name;
...
char is_minimig()
{
	// MinimigCD is a separate core built on the same Minimig chipset, so it needs
...
static int is_minimigcd_type = 0;
char is_minimigcd()
{
	// The simplified "console" Minimig variant (CONF_STR "MinimigCD"). Renders a
	...
	if (!is_minimigcd_type) is_minimigcd_type = strncasecmp(orig_name, "minimigcd", 7) ? 2 : 1;
	return (is_minimigcd_type == 1);
}
